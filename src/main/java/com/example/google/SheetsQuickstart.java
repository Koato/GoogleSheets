...
public class SheetsQuickstart {
	...
	public static void main(String... args) throws IOException, GeneralSecurityException {
		...
		ValueRange response = service.spreadsheets().values().get(spreadsheetId, range)
				.setDateTimeRenderOption("FORMATTED_STRING").setValueRenderOption("FORMATTED_VALUE").execute();
		...
		if (values == null || values.isEmpty()) {
			...
			for (List<Object> row : values) {
				fila++;
				if (fila == 1) {
					System.err.println(row.get(0) + " | " + row.get(1) + " | " + row.get(2) + " | " + row.get(3) + " | "
							+ row.get(4) + " | " + row.get(5));
				} else {
					System.out.print(row.get(0) + " | " + row.get(1) + " | ");
					System.out.print(row.get(2) + " | " + row.get(3) + " | ");
	...
	private static Credential getCredentials(final NetHttpTransport HTTP_TRANSPORT) throws IOException {
		...
		LocalServerReceiver receiver = new LocalServerReceiver.Builder().setPort(3030).build();
